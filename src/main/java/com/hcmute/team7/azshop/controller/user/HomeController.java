...
@WebServlet(urlPatterns = {"/home", "/buyer/login", "/buyer/register", "/buyer/forgot",
        "/buyer/waiting", "/buyer/verifyCode", "/buyer/logout"})
public class HomeController extends HttpServlet {
    ...
    protected void postRegister(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (userService.checkExistEmail(email)) {
            ...
            if (test) {
                ...
                boolean isSuccess = userService.Register(userName, fullName, email, code, password);

                if(isSuccess) {
                    response.sendRedirect("/buyer/verifyCode");
                } else {
    ...
    protected void getLogin(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (session != null && session.getAttribute("account") != null) {
            response.sendRedirect("/buyer/waiting");
            return;
        }
    ...
    protected void postVerifyCode(HttpServletRequest request) {
        String alertMsg = "";
        // Truy cập session
        HttpSession session = request.getSession();
        User user = (User) session.getAttribute("account");
