...
public class OrderServiceImpl implements IOrderService {
    ...
    public void create(Cart cart, Long storeId) {
        ...
        // Tạo Order mới trước để có ID hợp lệ cho các OrderItem
        orderDAO.create(order);

        List<OrderItem> orderItemList = new ArrayList<>();
        for (CartItem item : cart.getCartItems()) {
            OrderItem orderItem = new OrderItem();
            orderItem.setOrders(order); // Đảm bảo rằng Orders đã được set cho OrderItem
        ...
        // Cập nhật lại Orders để thêm các OrderItem
